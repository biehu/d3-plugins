 ...
 */
YXCharts.scalePie = {};
YXCharts.scalePie.defualts = {
	data: {}
}
YXCharts.scalePie.init = function (dom, options) {
    var w = 220;
    var h = 220;
    var margin = 20;
    var outerRadius = 90;
	...
	if (d3.select(dom).select('svg').size() > 0) {
    ...
    var h1Html = '';
    for (var i = 0; i < (+options.data.h1 + 1); i++) {
        h1Html += '<span>' + i + '</span>';
    }
    ...
    var bottomCount=svg.append('foreignObject')
        ...
        .attr('width', textWidth)
        .attr('x', -textWidth / 2)
        .html('<h2>' + options.data.h2 + '</h2>');
    var bottomText=svg.append('foreignObject')
        .attr('class', 'h3')
        .attr('width', textWidth)
        .attr('x', -textWidth / 2)
        .html('<h3>' + options.data.h3 + '</h3>');
